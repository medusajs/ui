...
<Command className="my-4">`yarn add @medusajs/medusa-ui`</Command>
...
<Command className="my-4">`yarn add -D @medusajs/ui-preset`</Command>

After you have installed Tailwind CSS and the Medusa UI preset, you need to add the following to your <Code>tailwind.config.js</Code> file:
...
<Code>@medusajs/ui</Code> is included in the content field of your <Code>tailwind.config.js</Code> file:
...
If you are working within a monorepo, you may need to add the path to the <Code>@medusajs/ui</Code> package in your <Code>tailwind.config.js</Code> like so:
